      ...
      <nav className={`fixed top-0 w-full z-50 transition-all duration-300 ${
        ...
        <div className={`grid grid-cols-2 md:grid-cols-3 items-center px-6 md:px-8 max-w-7xl mx-auto w-full transition-all duration-300 ${
          ...
          <div className={`justify-self-start text-2xl font-headline tracking-tight transition-colors duration-300 ${
            isScrolled ? "text-slate-900 dark:text-slate-50" : "text-white drop-shadow-md"
          }`}>
            Allie&apos;s Bar
          </div>
          ...
          <div className="justify-self-end flex items-center gap-4">
            ...
            <Link
              href="/pricing"
              className={`hidden md:block px-6 py-2 rounded-lg font-bold hover:opacity-80 active:scale-95 transition-all duration-300 ${
                isScrolled
            ...
            <button
              ...
              aria-expanded={open}
            >
              <span className={`block w-5 h-0.5 rounded-full transition-colors ${isScrolled ? "bg-slate-700" : "bg-white"}`} />
              <span className={`block w-5 h-0.5 rounded-full transition-colors ${isScrolled ? "bg-slate-700" : "bg-white"}`} />
              <span className={`block w-3.5 h-0.5 rounded-full self-start ml-[3px] transition-colors ${isScrolled ? "bg-slate-700" : "bg-white"}`} />
            </button>
          </div>
        </div>
      </nav>
